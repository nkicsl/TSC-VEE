...
void EVMCMessage_init_v(struct evmc_message* msg, int64_t gas, uint8_t* input, size_t input_size){
    ...
    TEE_MemFill(msg->recipient.bytes, 1, 20);
    TEE_MemFill(msg->sender.bytes, 0, 20);

    msg->input_data = input;
    msg->input_size = input_size;

    TEE_MemFill(msg->value.bytes, 0, 32);
    TEE_MemFill(msg->create2_salt.bytes, 0, 20);
    TEE_MemFill(msg->code_address.bytes, 0, 20);
}
...
void evmcContext_init(struct evmc_tx_context tx){
    TEE_MemFill(tx.tx_gas_price.bytes, 0, 32);
    TEE_MemFill(tx.tx_origin.bytes, 0, 20);
    TEE_MemFill(tx.block_coinbase.bytes, 0, 20);
    tx.block_number = 0;
    tx.block_timestamp = 0;
    tx.block_gas_limit = 0;
    TEE_MemFill(tx.block_prev_randao.bytes, 0, 32);
    TEE_MemFill(tx.chain_id.bytes, 0, 32);
    TEE_MemFill(tx.block_base_fee.bytes, 0, 32);
}
...
void evmc_result_init(struct evmc_result* res){
    ...
    res->output_size = 0;
    res->release = NULL;
    TEE_MemFill(res->create_address.bytes, 0, 20);
    TEE_MemFill(res->padding, 0, 4);
}
...
{
    return strcmp((char *)a->bytes, (char *)b->bytes);
}
...
int msg_cmp(const struct evmc_message *a, const struct evmc_message *b)
{
    ...
    return strcmp((char *)a->input_data, (char *)b->input_data);
}
...
struct evmc_result evmc_make_result(enum evmc_status_code status_code, int64_t gas_left, const uint8_t *output_data, size_t output_size)
{
    ...
    if (output_size != 0)
    {
        uint8_t* buffer = TEE_Malloc(output_size, TEE_MALLOC_FILL_ZERO);
        ...
        TEE_MemMove(buffer, output_data, output_size);
