...
enum evmc_status_code keccak256(struct StackTop stack, struct ExecutionState *state)
{
    ...
    uint8_t *hash_value = (uint8_t*)TEE_Malloc(32, TEE_MALLOC_FILL_ZERO);
    TEE_MemMove(hash_value, hash, 32);
...
{
    uint8_t *bytes;
    TEE_MemMove(bytes, state->msg->recipient.bytes, 20);
    uint8_t addr[32];
    for (int i = 20; i < 32; i++){
        addr[i] = 0;
    }
...
{
    uint8_t *bytes = (uint8_t*)TEE_Malloc(20, TEE_MALLOC_FILL_ZERO);
    TEE_MemMove(bytes, state->msg->sender.bytes, 20);
    uint8_t addr[32];
    TEE_MemFill(addr, 0, 32);
    for (int i = 19; i >= 0; i--)
    {
        addr[12 + i] = *(bytes + i);
    }
...
void callvalue(struct StackTop stack, struct ExecutionState *state)
{
    ...
    uint8_t tmp[32];
    TEE_MemMove(tmp, state->msg->value.bytes, 32);
...
enum evmc_status_code calldatacopy(struct StackTop stack, struct ExecutionState *state)
{
    ...
    if (copy_size > 0)
        TEE_MemMove(Memory_get(&state->memory, dst), &state->msg->input_data[src], copy_size);

    if (s - copy_size > 0)
        TEE_MemFill(Memory_get(&state->memory, dst + copy_size), 0, s - copy_size);
...
enum evmc_status_code codecopy(struct StackTop stack, struct ExecutionState *state)
{
    ...
    if (copy_size > 0)
        TEE_MemMove(Memory_get(&state->memory, dst), &state->code.data[src], copy_size);
...
enum evmc_status_code extcodecopy(struct StackTop stack, struct ExecutionState *state)
{
    ...
    if (s > 0)
    {
        ...
        if (num_bytes_to_clear > 0)
            TEE_MemFill(Memory_get(&state->memory, dst + num_bytes_copied), 0, num_bytes_to_clear);
    }
...
enum evmc_status_code returndatacopy(struct StackTop stack, struct ExecutionState *state)
{
    ...
    if (s > 0)
        TEE_MemMove(Memory_get(&state->memory, dst), &state->return_data[src], s);
    return EVMC_SUCCESS;
}
...
enum evmc_status_code call_(struct StackTop stack, struct ExecutionState* state){
    ...
    *StackTop_top(&stack) = (struct uint256_t){0, 0, 0, (uint64_t)(result.status_code == EVMC_SUCCESS)};
    ...
    if(copy_size > 0)
        TEE_MemMove(Memory_get(&state->memory, (size_t)LOWER(LOWER_P(output_offset))), result.output_data, copy_size);
...
enum evmc_status_code create(struct StackTop stack, struct ExecutionState* state){
    ...
    if((size_t)LOWER(LOWER_P(init_code_size)) > 0){
        ...
        TEE_MemMove(msg.input_data, Memory_get(&state->memory, (size_t)LOWER(LOWER_P(init_code_offset))), msg.input_size);
    }
    msg.sender = state->msg->recipient;
    msg.depth = state->msg->depth + 1;
    ...
    TEE_MemMove(state->return_data, result.output_data, result.output_size);
