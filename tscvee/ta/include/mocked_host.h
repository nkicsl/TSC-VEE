...
{
    TEE_MemFill(sv->value.bytes, 0, 32);
    sv->dirty = false;
...
{
    TEE_MemMove(sv->value.bytes, value.bytes, 32);
    sv->dirty = dirty;
    sv->access_status = EVMC_ACCESS_COLD;
}
...
void MockedAccount_init(struct MockedAccount* ma)
{
    ma->nonce = 0;
    ma->code = cstr_init();
    TEE_MemFill(ma->codehash.bytes, 0, 32);
    TEE_MemFill(ma->balance.bytes, 0, 32);
...
void log_record_init(struct log_record* lr){
    TEE_MemFill(lr->creator.bytes, 0, 20);
    lr->data = cstr_init();
...
void selfdestruct_record_init(struct selfdestruct_record* sr){
    TEE_MemFill(sr->selfdestructed.bytes, 0, 20);
    TEE_MemFill(sr->beneficiary.bytes, 0, 20);
}
...
{   
    ...
    TEE_MemFill(mockhost->block_hash.bytes, 0, 32);
...
enum evmc_storage_status set_storage(MockedHost *mockhost, evmc_address addr, evmc_bytes32 key, evmc_bytes32 value)
{
    ...
    TEE_MemMove(old->value.bytes, value.bytes, 32);
    return status;
}
